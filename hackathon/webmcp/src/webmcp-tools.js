...
export function createWebMCPTools(curriculumData, customStateStore = null) {
  ...
  function findTopic(topic_id) {
    for (const chapter of CATALOG.chapters || []) {
      const topic = (chapter.topics || []).find(t => t.id === topic_id);
      if (topic) return { topic, chapter, dataPath: topic.data_path };
    }
    if (CATALOG.foundations) {
      const topic = (CATALOG.foundations.topics || []).find(t => t.id === topic_id);
  ...
  async function fetchTopicJson(topic_id) {
    ...
    const urlPath = found.dataPath.startsWith("/") ? found.dataPath : "/" + found.dataPath;
    let json = null;
    try {
      const response = await fetch(urlPath);
      if (response.ok) json = await response.json();
    } catch (e) { /* fall through to Node fs fallback */ }
  ...
  function findQuestion(topicJson, question_id) {
    ...
    for (const qt of topicJson.question_types) {
      const q = (qt.pool || []).find(item => item.id === question_id);
      if (q) return q;
    }
    return null;
  }
  ...
  function getTopicMetadata(params = {}) {
    ...
    for (const chapter of CATALOG.chapters || []) {
      const topic = (chapter.topics || []).find(t => t.id === topic_id);
      if (topic) return { chapter_id: chapter.id, chapter_title: chapter.title, topic: topic };
    }
    if (CATALOG.foundations) {
      const foundTopic = (CATALOG.foundations.topics || []).find(t => t.id === topic_id);
      if (foundTopic) return { chapter_id: "foundations", chapter_title: CATALOG.foundations.description, topic: foundTopic };
    }
  ...
  async function getTopicContent(params = {}) {
    ...
    const result = {
      ...
      concepts: includeSolutions ? (fullContent?.concepts || null) : null,
      concept_count: fullContent?.concepts ? fullContent.concepts.length : 0,
      worked_examples: includeSolutions ? (fullContent?.worked_examples || null) : null,
  ...
  async function getPrerequisiteCheck(params = {}) {
    const topic_id = params.topic_id;
    if (!topic_id) throw new Error("Parameter \"topic_id\" is required.");
    const found = findTopic(topic_id);
    if (!found) throw new Error(`Topic "${topic_id}" not found.`);
    ...
    const withStatus = prerequisites.map(p => ({
      ...p,
      mastered: mastered.includes(p.id) || completedTopics.includes(p.id) || completedChapters.includes(p.id)
    }));
