...
async function translateConcepts(conceptsJson) {
    ...
    const bilingualConcepts = { sections: [], upscNotes: [], keyTakeaways: [] };
    ...
    for (let s = 0; s < conceptsJson.sections.length; s++) {
        ...
        const sectionPrompt = `You are a professional English-to-Hindi translator specializing in Environmental & Social Studies notes for the UP Assistant Teacher exam. Translate the following single section of exam prep notes into a BILINGUAL format.
...
Keep structural keys and layout values (like "type": "table", "type": "list", "type": "subcards") as simple strings. Preserve all markdown formatting.
...
${JSON.stringify(section, null, 2)}`;
    ...
    if (conceptsJson.upscNotes && conceptsJson.upscNotes.length > 0) {
        ...
        const notesPrompt = `You are a professional English-to-Hindi translator for Environmental & Social Studies notes. Translate the following list of notes/tips into a BILINGUAL format.
...
Keep the "type" key as a simple string. Preserve markdown.
...
${JSON.stringify(conceptsJson.upscNotes, null, 2)}`;
    ...
    if (conceptsJson.keyTakeaways && conceptsJson.keyTakeaways.length > 0) {
        ...
        const takeawaysPrompt = `You are a professional English-to-Hindi translator for Environmental & Social Studies notes. Translate the following list of key takeaways into a BILINGUAL format.
Convert each string into a bilingual object:
{
...
// ============================================================================
async function main() {
    const targetDir = path.join(process.cwd(), 'up-assistant-teacher', 'environmental-social-studies');
    ...
    console.log(`Found ${topics.length} Environmental & Social Studies topics to translate.`);
    ...
    for (let i = 0; i < topicsToProcess.length; i++) {
        ...
        console.log(`\n[${i + 1}/${topicsToProcess.length}] Translating Environmental & Social Studies topic: ${topic}`);
    ...
    console.log('\n🎉 Environmental & Social Studies translation process complete!');
}
