...
def verify_file(filepath, name):
    ...
    if "title" not in data["deepDive"] or "description" not in data["deepDive"]:
        print("ERROR: 'title' or 'description' missing in deepDive!")
        return False
    ...
    pq_list = data.get("practiceQuestions", [])
    if len(pq_list) != 50:
        print(f"ERROR: Expected 50 practice questions, found {len(pq_list)}")
        success = False
    else:
        pq_keys_ok = True
        for idx, q in enumerate(pq_list):
            missing = required_keys - set(q.keys())
            if missing:
                print(f"ERROR: practiceQuestions[{idx}] is missing keys: {missing}")
                pq_keys_ok = False
                success = False
        if pq_keys_ok:
            print("\nPractice Questions: 50 (Keys OK) -> PASS")
        else:
            print("\nPractice Questions: FAILED key validation")
        
    mq_list = data.get("mockTestQuestions", [])
    if len(mq_list) != 10:
        print(f"ERROR: Expected 10 mock questions, found {len(mq_list)}")
        success = False
    else:
        mq_keys_ok = True
        for idx, q in enumerate(mq_list):
            missing = required_keys - set(q.keys())
            if missing:
                print(f"ERROR: mockTestQuestions[{idx}] is missing keys: {missing}")
                mq_keys_ok = False
                success = False
        if mq_keys_ok:
            print("Mock Questions: 10 (Keys OK) -> PASS")
        else:
            print("Mock Questions: FAILED key validation")
