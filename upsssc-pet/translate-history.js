...
async function translateConcepts(conceptsJson) {
    ...
    for (let s = 0; s < conceptsJson.sections.length; s++) {
        ...
        const sectionPrompt = `You are a professional English-to-Hindi translator specializing in History exam prep notes. Translate the following single section of exam prep notes into a BILINGUAL format.
...
${JSON.stringify(section, null, 2)}`;
    ...
    if (conceptsJson.upscNotes && conceptsJson.upscNotes.length > 0) {
        ...
        const notesPrompt = `You are a professional English-to-Hindi translator specializing in History notes. Translate the following list of notes/tips into a BILINGUAL format.
...
${JSON.stringify(conceptsJson.upscNotes, null, 2)}`;
    ...
    if (conceptsJson.keyTakeaways && conceptsJson.keyTakeaways.length > 0) {
        ...
        const takeawaysPrompt = `You are a professional English-to-Hindi translator specializing in History notes. Translate the following list of key takeaways into a BILINGUAL format.
...
// ============================================================================
async function main() {
    const targetDir = path.join(process.cwd(), 'upsssc-pet', 'history');
    ...
    console.log(`Found ${topics.length} History topics to translate.`);
    ...
    for (let i = 0; i < topicsToProcess.length; i++) {
        ...
        console.log(`\n[${i + 1}/${topicsToProcess.length}] Translating History topic: ${topic}`);
    ...
    console.log('\n🎉 History translation process complete!');
}
