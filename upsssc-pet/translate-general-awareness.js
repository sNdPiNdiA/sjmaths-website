...
// CONSTANTS
// ============================================================================
const REQUEST_DELAY_MS = 10000; // 10s between API calls to avoid rate limiting
const MAX_RETRIES = 5;
let currentModel = 'gemini-2.5-flash'; // Good for structured translation tasks
...
function parseResponse(raw) {
    ...
    try { return JSON.parse(cleaned); } catch (err) {
        ...
        // Find position if mentioned in error message
        const posMatch = err.message.match(/position (\d+)/);
        if (posMatch) {
            const pos = parseInt(posMatch[1], 10);
            console.error("Context around position:", cleaned.substring(Math.max(0, pos - 100), Math.min(cleaned.length, pos + 100)));
        }
    }
...
async function translateConcepts(conceptsJson) {
    ...
    for (let s = 0; s < conceptsJson.sections.length; s++) {
        ...
        const sectionPrompt = `You are a professional English-to-Hindi translator. Translate the following single section of exam prep notes into a BILINGUAL format.
...
${JSON.stringify(section, null, 2)}`;
        ...
        await sleep(2000); // Small sleep between chunk calls to be safe
    }

    // 2. Translate upscNotes
    if (conceptsJson.upscNotes && conceptsJson.upscNotes.length > 0) {
        console.log(`      🔸 Translating upscNotes...`);
        const notesPrompt = `You are a professional English-to-Hindi translator. Translate the following list of notes/tips into a BILINGUAL format.
...
${JSON.stringify(conceptsJson.upscNotes, null, 2)}`;
    ...
    if (conceptsJson.keyTakeaways && conceptsJson.keyTakeaways.length > 0) {
        ...
        const takeawaysPrompt = `You are a professional English-to-Hindi translator. Translate the following list of key takeaways into a BILINGUAL format.
...
// ============================================================================
async function main() {
    const targetDir = path.join(process.cwd(), 'upsssc-pet', 'general-awareness');
    ...
    console.log(`Found ${topics.length} topics to translate.`);

    // If a topic arg is passed, only run on that topic (e.g., node translate.js awards-winners)
    ...
    for (let i = 0; i < topicsToProcess.length; i++) {
        ...
        console.log(`\n[${i + 1}/${topicsToProcess.length}] Translating topic: ${topic}`);
