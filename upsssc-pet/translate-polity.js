...
async function translateConcepts(conceptsJson) {
    ...
    for (let s = 0; s < conceptsJson.sections.length; s++) {
        ...
        const sectionPrompt = `You are a professional English-to-Hindi translator specializing in Polity exam prep notes. Translate the following single section of exam prep notes into a BILINGUAL format.
...
${JSON.stringify(section, null, 2)}`;
    ...
    if (conceptsJson.upscNotes && conceptsJson.upscNotes.length > 0) {
        ...
        const notesPrompt = `You are a professional English-to-Hindi translator specializing in Polity notes. Translate the following list of notes/tips into a BILINGUAL format.
...
${JSON.stringify(conceptsJson.upscNotes, null, 2)}`;
    ...
    if (conceptsJson.keyTakeaways && conceptsJson.keyTakeaways.length > 0) {
        ...
        const takeawaysPrompt = `You are a professional English-to-Hindi translator specializing in Polity notes. Translate the following list of key takeaways into a BILINGUAL format.
...
// ============================================================================
async function main() {
    const targetDir = path.join(process.cwd(), 'upsssc-pet', 'polity');
    ...
    console.log(`Found ${topics.length} Polity topics to translate.`);
    ...
    for (let i = 0; i < topicsToProcess.length; i++) {
        ...
        console.log(`\n[${i + 1}/${topicsToProcess.length}] Translating Polity topic: ${topic}`);
    ...
    console.log('\n🎉 Polity translation process complete!');
}
