...
// CONSTANTS
// ============================================================================
const REQUEST_DELAY_MS = 10000; // 10s delay between calls
const MAX_RETRIES = 5;
    ...
    for (let attempt = 1; attempt <= retries; attempt++) {
        try {
            ...
            if (res.status === 429 || res.status === 403) {
                ...
                const wait = 10000 * Math.pow(2, attempt - 1);
...
function parseResponse(raw) {
    ...
    const start = (jsonStart >= 0 && (jsonStartArr === -1 || jsonStart < jsonStartArr)) ? jsonStart : jsonStartArr;
...
// ============================================================================
// PROMPT BUILDERS FOR ALL 4 TABS
// ============================================================================

// --- TAB 1: CONCEPTS & THEORIES, TIPS AND TRICKS ---
function buildConceptsPrompt(topic) {
    return `You are a Senior English Grammar Faculty for UPSSSC PET and competitive exams.
Create ULTRA-COMPREHENSIVE, EXAM-FOCUSED concept notes with tips and tricks for the topic: "${topic.name}" (${topic.hindiName}).

TOPIC CONTEXT:
- Subject: General English / English Grammar
- Exam: UPSSSC PET (Uttar Pradesh Subordinate Services Selection Commission)
- Keywords: ${topic.keywords.join(', ')}

CRITICAL FORMAT RULES:
1. STRICTLY NO PARAGRAPHS — Every section must be a table, subcards, or list.
2. Every rule must have clear explanation and 5+ high-quality exam examples.
3. Include memory tricks, shortcuts, and golden rules for spotting errors.

REQUIRED STRUCTURE (Return ONLY valid JSON):
{
  "sections": [
    {
      ...
      "rows": [
        ["Definition & Scope", "**Core definition** and relevance for UPSSSC PET"],
        ["Key Exam Areas", "Error Spotting, Sentence Improvement, Fill in the Blanks"]
      ]
    },
    {
      ...
      "items": [
        {
          "title": "Golden Rule 1 with **Trick**",
          "content": "• Explanation of rule with **bold key terms**\\n• Example 1\\n• Example 2\\n• **Tip/Trick:** Short trick to remember"
        }
      ]
    },
    {
      ...
      "type": "table",
      "headers": ["Category / Condition", "Grammatical Rule", "Correct Usage Examples"],
      "rows": [
        ["Rule Category 1", "Specific rule detail", "Example sentence"]
      ]
    },
    {
      ...
      "items": [
        {
          "term": "Trick 1: Shortcut Title",
          "definition": "Detailed trick explanation with **bold** key rules"
        }
      ]
    },
    {
      ...
      "items": [
        {
          "term": "Common Error 1",
          "definition": "Incorrect vs Correct usage and why students get confused"
        }
      ]
    },
    {
      ...
      "items": [
        {
          "term": "Key Takeaway 1",
          "definition": "Concise summary point for rapid revision"
        }
      ]
    }
  ],
  "upscNotes": [
    { "type": "tip", "content": "Exam strategy tip for UPSSSC PET English section" },
    { "type": "trap", "content": "Common trap option set by examiners in MCQs" }
  ],
  "keyTakeaways": [
...
function buildPracticePrompt(topic) {
    return `You are a Senior English Exam Setter. Generate 15 HIGH-YIELD Practice Questions for UPSSSC PET on the topic: "${topic.name}".
Cover error spotting, fill in the blanks, sentence improvement, and direct/indirect transformations.

Return ONLY a valid JSON array of 15 objects with this exact structure:
[
  {
    "id": 1,
...
Important: "correct" index must be 0, 1, 2, or 3.`;
}

// --- TAB 3: PREVIOUS YEAR QUESTIONS (PYQs) ---
function buildPyqsPrompt(topic) {
    return `You are an expert on UPSSSC PET and UP state exam PYQs. Generate 10 authentic Previous Year Questions (PYQs) from UPSSSC PET / UPSSSC exams (2021-2025) on the topic: "${topic.name}".

Return ONLY a valid JSON array of 10 objects with this exact structure:
[
  {
    "id": 1,
    "year": "UPSSSC PET 2023",
    "question": "Question text here...",
    "options": ["Option A", "Option B", "Option C", "Option D"],
    "correct": 1,
    "explanation": "Detailed step-by-step solution explaining the grammar rule tested."
  }
]
Important: "correct" index must be 0, 1, 2, or 3. Use years like "UPSSSC PET 2021", "UPSSSC PET 2022", "UPSSSC PET 2023", "UPSSSC VDO 2023", "UPSSSC PET 2024".`;
}

// --- TAB 4: MINI TEST ---
function buildTestPrompt(topic) {
    return `You are a Test Series Developer. Generate a 10-Question Timed Mini Test for UPSSSC PET on the topic: "${topic.name}".
...
[
  {
    ...
    "options": ["Option A", "Option B", "Option C", "Option D"],
    "correct": 2,
    "explanation": "Clear and concise explanation of the answer."
  }
]
Important: "correct" index must be 0, 1, 2, or 3.`;
}
...
// ============================================================================
function assemblePage(topic, conceptsData, practiceData, pyqsData, testData) {
...
// ============================================================================
// FALLBACK GENERATORS (for maximum stability)
// ============================================================================
        ...
        sections: [
            {
                ...
                rows: [
                    ...
                    ['Exam Scope', 'Error Spotting, Sentence Improvement, Fillers, and Vocabulary']
                ]
            }
        ],
        upscNotes: [
            { type: 'tip', content: `Master fundamental rules of ${topic.name} for UPSSSC PET.` }
        ],
        keyTakeaways: [
            `Revise all core rules of ${topic.name} before attempting practice questions.`
        ]
    };
}

function buildFallbackPractice(topic) {
    return [
        {
            id: 1,
            question: `Choose the correct grammatical form related to ${topic.name}:`,
            options: ['Option A', 'Option B', 'Option C', 'Option D'],
            correct: 0,
            explanation: `Option A correctly follows the fundamental rule of ${topic.name}.`
        }
    ];
}

function buildFallbackPyqs(topic) {
    return [
        {
            id: 1,
            year: 'UPSSSC PET 2023',
            question: `Identify the correct answer for ${topic.name}:`,
            options: ['Option A', 'Option B', 'Option C', 'Option D'],
            correct: 1,
            explanation: `Option B is the correct answer according to UPSSSC exam standards.`
        }
    ];
}

function buildFallbackTest(topic) {
    return [
        {
            id: 1,
            question: `Test question on ${topic.name}:`,
            options: ['Option A', 'Option B', 'Option C', 'Option D'],
            correct: 0,
...
async function main() {
    console.log('╔══════════════════════════════════════════════════════════════╗');
    console.log('║ UPSSSC PET English Grammar — 4-Tab All-Files Generator      ║');
    ...
    for (let i = 0; i < totalTopics; i++) {
        ...
        console.log(`\n${'='.repeat(80)}`);
        console.log(`[${i + 1}/${totalTopics}] Generating All Tabs for: ${topic.name} (${topic.hindiName})`);
        ...
        // 1. Generate Tab 1: Concepts & Theories, Tips and Tricks
        try {
            console.log('  📝 [Tab 1/4] Generating Concepts & Theories, Tips & Tricks...');
            const prompt = buildConceptsPrompt(topic);
            const raw = await callGemini(prompt);
            conceptsData = parseResponse(raw);
            console.log('  ✅ Tab 1 generated successfully!');
        } catch (err) {
            console.error(`  ⚠️ Tab 1 fallback used: ${err.message}`);
        ...
        // 2. Generate Tab 2: Practice Questions
        try {
            console.log('  📝 [Tab 2/4] Generating Practice Questions...');
            const prompt = buildPracticePrompt(topic);
            const raw = await callGemini(prompt);
            practiceData = parseResponse(raw);
            console.log('  ✅ Tab 2 generated successfully!');
        } catch (err) {
            console.error(`  ⚠️ Tab 2 fallback used: ${err.message}`);
            practiceData = buildFallbackPractice(topic);
        }
        fs.writeFileSync(path.join(tabsDir, 'practice.json'), JSON.stringify(practiceData, null, 2), 'utf8');
        console.log('  💾 Saved tabs/practice.json');
        await sleep(2000);

        // 3. Generate Tab 3: PYQs
        try {
            console.log('  📝 [Tab 3/4] Generating PYQs...');
            const prompt = buildPyqsPrompt(topic);
            const raw = await callGemini(prompt);
            pyqsData = parseResponse(raw);
            console.log('  ✅ Tab 3 generated successfully!');
        } catch (err) {
            console.error(`  ⚠️ Tab 3 fallback used: ${err.message}`);
            pyqsData = buildFallbackPyqs(topic);
        }
        fs.writeFileSync(path.join(tabsDir, 'pyqs.json'), JSON.stringify(pyqsData, null, 2), 'utf8');
        console.log('  💾 Saved tabs/pyqs.json');
        await sleep(2000);

        // 4. Generate Tab 4: Mini Test
        try {
            console.log('  📝 [Tab 4/4] Generating Mini Test...');
            const prompt = buildTestPrompt(topic);
            const raw = await callGemini(prompt);
            testData = parseResponse(raw);
            console.log('  ✅ Tab 4 generated successfully!');
        } catch (err) {
            console.error(`  ⚠️ Tab 4 fallback used: ${err.message}`);
            testData = buildFallbackTest(topic);
        }
        fs.writeFileSync(path.join(tabsDir, 'test.json'), JSON.stringify(testData, null, 2), 'utf8');
        console.log('  💾 Saved tabs/test.json');

        // 5. Save combined data.json
        const combinedData = {
        ...
        fs.writeFileSync(path.join(outputDir, 'data.json'), JSON.stringify(combinedData, null, 2), 'utf8');
        console.log('  💾 Saved data.json');

        // 6. Assemble and save index.html
        const html = assemblePage(topic, conceptsData, practiceData, pyqsData, testData);
        fs.writeFileSync(path.join(outputDir, 'index.html'), html, 'utf8');
        console.log('  💾 Saved index.html');

        console.log(`  🎉 Completed Topic ${i + 1}/${totalTopics}: ${topic.name}`);
        successCount++;
    ...
    console.log(`\n${'='.repeat(80)}`);
    console.log(`📊 SUMMARY: ${successCount} succeeded out of ${totalTopics} topics`);
    console.log(`${'='.repeat(80)}`);
    console.log('\nAll UPSSSC PET English Grammar tabwise files generated successfully!');
}
