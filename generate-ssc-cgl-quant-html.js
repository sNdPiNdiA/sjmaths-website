...
// Call Gemini API strictly using gemini-3.5-flash-lite with fallback models if needed
...
// Automatically scan ssc-cgl/quantitative-aptitude for all microtopic directories needing 5-tab generation
function discoverAllQuantTopics() {
        ...
        list.forEach(file => {
            ...
            if (stat.isDirectory()) {
                ...
                if (fs.existsSync(indexFile)) {
                    ...
                    const relativePath = path.relative(quantDir, fullPath).replace(/\\/g, '/');
...
const topicsToGenerate = discoverAllQuantTopics();
console.log(`📋 Total pending microtopic pages discovered: ${topicsToGenerate.length}`);

function buildPrompt(topicName) {
    return `You are the Senior Mathematics Faculty and Content Team of SJMaths.
Your task is to generate complete, high-quality, BILINGUAL (English & Hindi) educational content for SSC CGL Mathematics on the topic: "${topicName}".
...
[TAB1_START]
Content for Tab 1: Comprehensive Theory & Basics (Bilingual)
(Includes: Quick Overview, Learning Objectives, Concept Introduction, Real-Life Applications, Definitions, Important Terms Table, Formula Sheet Table, Derivations, Common Mistakes - All with .lang-en and .lang-hi wrappers)
[TAB1_END]
...
[TAB3_START]
Content for Tab 3: Revision Notes & FAQs (Bilingual)
(Includes: 1-Page Revision Notes, One Minute Revision bullets, Conceptual Interview Style Questions, Frequently Asked Questions in both .lang-en and .lang-hi - Do NOT use the term "SEO-Friendly", Internal Links, External References, Conclusion)
[TAB3_END]
...
<div class="mcq-card">
  <div class="mcq-question">
    <div class="lang-en"><strong>Q1.</strong> Question in English</div>
    <div class="lang-hi"><strong>Q1.</strong> प्रश्न हिंदी में</div>
  </div>
...
[TAB4_START]
Content for Tab 4: Previous Year Questions (PYQs) (Bilingual)
...
(Includes: 1-Page Revision Notes, One Minute Revision bullets, Conceptual Interview Style Questions, 15 SEO-friendly FAQs in both .lang-en and .lang-hi, Internal Links, External References, Conclusion)
[TAB5_END]
...
function parseTabs(aiText) {
    ...
    return {
        ...
        tab3: getSection('\\[TAB3_START\\]', '\\[TAB3_END\\]') || '<p>Practice MCQs content loading...</p>',
        tab4: getSection('\\[TAB4_START\\]', '\\[TAB4_END\\]') || '<p>Previous Year Questions loading...</p>',
        tab5: getSection('\\[TAB5_START\\]', '\\[TAB5_END\\]') || '<p>Revision Notes & FAQs loading...</p>'
    };
}
...
<head>
    ...
    <title>${topicName} - SSC CGL Mathematics | SJMaths</title>
    <meta name="description" content="Master ${topicName} for SSC CGL Tier 1 & Tier 2. Complete concept notes, formulas, shortcuts, PYQs, practice questions, and FAQs.">
    <meta name="robots" content="index, follow, max-image-preview:large">
    <link rel="canonical" href="https://sjmaths.com/ssc-cgl/quantitative-aptitude/${relativeUrlPath}/">
    <meta name="keywords" content="${topicName}, SSC CGL Maths, Quantitative Aptitude, SSC CGL Tier 1, SSC CGL Tier 2, SJMaths">
    ...
    <style>
        ...
        /* Subject Navigation Menu - Mobile Touch Scroll */
        .subject-nav {
            display: flex;
            gap: 0.4rem;
            margin-bottom: 1.25rem;
        ...
        /* Bilingual Language Toggle Integration (Synced with Global Header) */
        .lang-hi {
            display: none;
        }
...
<body>
    ...
    <main class="topic-container" id="main-content">
        ...
        <div class="breadcrumbs">
            ...
            <a href="/ssc-cgl/quantitative-aptitude/">Quantitative Aptitude</a> <i class="fas fa-chevron-right" style="font-size: 0.65rem; margin: 0 0.2rem;"></i>
            <span>${topicName}</span>
        </div>
        ...
        <div class="topic-header">
            <h1>${topicName}</h1>
            <p>Complete 5-Tab SSC CGL Guide: Theory, Shortcuts, Practice MCQs, PYQs & Revision Notes.</p>
        </div>
...
async function runGenerator() {
    console.log('🚀 Starting SSC CGL Quantitative Aptitude Mobile-First 5-Tab HTML Generator...');
    
    for (const item of topicsToGenerate) {
        console.log(`\n⏳ Generating content for: ${item.topicName}`);
        const prompt = buildPrompt(item.topicName);
        
        try {
            const aiResponse = await callGemini(prompt);
            const parsedTabs = parseTabs(aiResponse);
            
            const htmlContent = generateSingleHTMLPage(item.topicName, item.categoryDir, parsedTabs);
            
            const targetDir = path.join('c:/Users/sande/Documents/GitHub/sjmaths-website/ssc-cgl/quantitative-aptitude', item.categoryDir);
            ...
            console.error(`❌ Failed for topic: ${item.topicName}`, err.message);
        }
