...
// Call Gemini API using gemini-3.6-flash as primary with fallbacks
...
const economyTopics = [
    ...
    "NITI Aayog Structure & Cooperative Federalism",
    "Money Supply Measures (M1-M4) & Inflation Types",
    "RBI Monetary Tools (Repo, CRR, SLR) & NPAs",
    "Indian Taxation System, GST Councils & Slabs",
    "Union Budget, Accounts & Deficit Metrics",
    "Sectors: Agriculture, MSMEs & Services",
...
5. CONTENT STRUCTURE for Tab 1 (Concept & Theory) - generate ALL of these sections that are applicable:
   ...
   - Key Formulas / Data / Statistics / Policy Highlights (table format with values, what they mean, Hindi meaning)
...
6. CRITICAL MATH RULE — LaTeX formulas in lang-en sections:
   - Use LaTeX syntax: inline $formula$ and display $$formula$$.
   - ONLY use English letters and math symbols inside LaTeX. Example: $GDP_{MP}$, $$NNP_{FC} = GNP_{MP} - \\text{Depreciation}$$
   - NEVER put Hindi/Devanagari text inside LaTeX \\text{} or any LaTeX command.
   - WRONG: $$\\text{सकल घरेलू उत्पाद} = C + I + G + NX$$
   - CORRECT: $$GDP_{MP} = C + I + G + NX$$  (use abbreviations, not Hindi words in LaTeX)
7. CRITICAL MATH RULE — lang-hi sections:
   - In .lang-hi divs/spans, do NOT copy the LaTeX $...$ formula from the English section.
   - Instead, write the formula as PLAIN TEXT with Unicode math symbols: ×, +, −, =, ≥, ≤, Δ, Σ
   - Use <sub> and <sup> tags for subscripts/superscripts in Hindi sections.
   - Example for Hindi section: <p class="lang-hi">सकल घरेलू उत्पाद<sub>MP</sub> = C + I + G + NX</p>`;
}
...
<head>
    ...
    <style>
        ...
        .coming-soon-box {
            text-align: center;
            padding: 3rem 1.5rem;
            background: linear-gradient(135deg, rgba(39, 174, 96, 0.03), rgba(41, 128, 185, 0.03));
            border-radius: 1rem;
...
async function runGenerator() {
    console.log('🚀 Starting SSC CGL Economy 4-Tab HTML Generator...');
    console.log(`📋 Total topics to generate: ${economyTopics.length}\n`);

    for (let i = 0; i < economyTopics.length; i++) {
        const topicName = economyTopics[i];
        const slug = slugify(topicName.replace(/&/g, 'and'));
        console.log(`⏳ [${i + 1}/${economyTopics.length}] Generating: ${topicName}`);
