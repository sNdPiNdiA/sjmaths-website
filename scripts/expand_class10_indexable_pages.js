...
const targetSubdirs = [
  path.join(CLASS10_DIR, 'ncert-exercise-practice'),
  path.join(CLASS10_DIR, 'worksheets'),
  path.join(CLASS10_DIR, 'tests', 'chapter-wise')
];
...
console.log(`Found ${unlockedFiles.length} target pages to unlock for indexing.`);
...
for (const filePath of unlockedFiles) {
  ...
  if (!title || title.length < 25 || title.includes('Updating')) {
    ...
    if (relPath.includes('ncert-exercise-practice')) {
      ...
      cleanTitle = `Class 10 Maths Chapter ${chapName} Exercise ${exNum} NCERT Solutions | SJMaths`;
    } else if (relPath.includes('worksheets')) {
      const type = filename.replace(/-/g, ' ').replace(/\b\w/g, c => c.toUpperCase());
      cleanTitle = `Class 10 ${chapName} ${type} Worksheet Solutions | SJMaths`;
    } else if (relPath.includes('tests')) {
      const testName = filename.replace(/-/g, ' ').replace(/\b\w/g, c => c.toUpperCase());
      cleanTitle = `Class 10 ${chapName} - ${testName} Online Test | SJMaths`;
    }
  ...
  if (!desc || desc.length < 50 || desc.length > 170) {
    ...
    desc = `Practice ${topicText} for Class 10 CBSE Board exam preparation. Access free step-by-step solutions, key questions, and expert revision notes on SJMaths.`;
    if (descM) {
...
console.log(`🎉 Unlocked and SEO-optimized ${count} Class 10 pages for search indexation.`);
