...
const CLASS_DIR = path.join(ROOT, 'class-11-maths');
const DOMAIN = 'https://sjmaths.com';
const IMAGE = `${DOMAIN}/assets/icons/icon-512x512.png`;

const chapterNames = {
  'chapter-1-sets': 'Sets',
  'chapter-2-relations-functions': 'Relations and Functions',
  'chapter-3-trigonometric-functions': 'Trigonometric Functions',
  'chapter-4-complex-numbers-and-quadratic-equations': 'Complex Numbers and Quadratic Equations',
  'chapter-5-linear-inequalities': 'Linear Inequalities',
  'chapter-6-permutations-and-combinations': 'Permutations and Combinations',
  'chapter-7-binomial-theorem': 'Binomial Theorem',
  'chapter-8-sequences-and-series': 'Sequences and Series',
  'chapter-9-straight-lines': 'Straight Lines',
  'chapter-10-conic-sections': 'Conic Sections',
  'chapter-11-three-dimensional-geometry': 'Three Dimensional Geometry',
  'chapter-12-limits-and-derivatives': 'Limits and Derivatives',
...
const shortChapterNames = {
  ...
  'Linear Inequalities': 'Linear Ineqs',
  'Permutations and Combinations': 'Permutations & Comb',
  'Sequences and Series': 'Sequences & Series',
  'Three Dimensional Geometry': '3D Geometry',
  'Limits and Derivatives': 'Limits & Derivatives',
};
...
function pageProfile(relativePath, content) {
  ...
  let keywords = ['Class 11 Maths', 'CBSE Class 11 Maths', 'NCERT Class 11 Maths', 'SJMaths'];
  let type = 'LearningResource';

  if (relativePath === 'class-11-maths/index.html') {
    ...
    keywords.push('Class 11 Maths NCERT Solutions', 'Class 11 Maths Notes', 'Class 11 Maths Worksheets');
    type = 'CollectionPage';
  } else if (relativePath.includes('/chapter-wise-notes/')) {
    title = chapter ? `Class 11 ${titleChapter} Notes | CBSE Maths` : 'Class 11 Maths Chapter Wise Notes | SJMaths';
    description = chapter
      ? `Class 11 Maths ${titleChapter} notes with key formulas, concepts, solved examples and chapter-end practice questions.`
      : 'Class 11 Maths chapter wise revision notes with formulas, solved examples and practice questions.';
    keywords.push('Class 11 Maths Notes', `${chapter} Class 11`, `${chapter} Notes`);
  } else if (relativePath.includes('/ncert-exercise-practice/') || relativePath.includes('/ncert-exemplar-practice/')) {
    let exercise = titleCaseSlug(file);
    if (exercise.includes('Miscellaneous')) {
      exercise = exercise.replace('Miscellaneous', 'Misc');
    }
    const isExemplar = relativePath.includes('exemplar');
    title = chapter ? `Class 11 ${titleChapter} ${exercise} ${isExemplar ? 'Exemplar ' : ''}Solutions` : 'Class 11 NCERT Maths Exercise Solutions';
    description = chapter
      ? `Practice Class 11 Maths ${titleChapter} ${exercise} ${isExemplar ? 'NCERT Exemplar ' : ''}solutions for exam preparation and concept mastery.`
      : 'Practice Class 11 Maths NCERT exercise questions with step-by-step solutions.';
    keywords.push('Class 11 NCERT Solutions', `${chapter} NCERT Solutions`, `${exercise} Class 11 Maths`);
    ...
    title = chapter ? `Class 11 ${titleChapter} ${worksheetType}` : 'Class 11 Maths Worksheets';
    description = chapter
      ? `Download and practice Class 11 Maths ${titleChapter} ${worksheetType.toLowerCase()} questions for exam preparation and revision.`
      : 'Download and practice Class 11 Maths worksheets for exam preparation and revision.';
    keywords.push('Class 11 Maths Worksheets', `${chapter} Worksheet`, `${worksheetType} Class 11 Maths`);
  } else if (relativePath.includes('/tests/')) {
    const testName = cleanTestName(file, chapter);
    title = chapter ? `Class 11 ${titleChapter} ${testName}` : `Class 11 Maths ${testName}`;
    description = chapter
      ? `Attempt Class 11 Maths ${titleChapter} ${testName.toLowerCase()} to test your understanding with CBSE-style practice questions.`
      : `Attempt ${testName} for Class 11 Maths CBSE board preparation.`;
    keywords.push('Class 11 Maths Test', `${chapter} Test`, 'CBSE Maths Practice Test');
  }
...
function upsertLearningResource(content, profile) {
  const marker = '<!-- SEO LearningResource: Class 11 -->';
  const schema = {
    ...
    about: ['Class 11 Maths', 'CBSE Mathematics'],
    publisher: {
      '@type': 'Organization',
      name: 'SJMaths',
      url: DOMAIN,
...
console.log(`Optimized ${changed} files across ${indexable} indexable Class 11 pages.`);
