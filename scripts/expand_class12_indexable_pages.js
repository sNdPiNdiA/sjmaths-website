...
const targetSubdirs = [
  path.join(CLASS12_DIR, 'ncert-exercise-practice'),
  path.join(CLASS12_DIR, 'previous-years-questions-chapter-wise'),
  path.join(CLASS12_DIR, 'tests'),
  path.join(CLASS12_DIR, 'sample-papers')
];
...
console.log(`Found ${unlockedFiles.length} Class 12 target pages to unlock for indexing.`);
...
for (const filePath of unlockedFiles) {
  ...
  if (!title || title.length < 25 || title.includes('Updating')) {
    ...
    if (relPath.includes('ncert-exercise-practice')) {
      ...
      cleanTitle = `Class 12 Maths Chapter ${chapName} Exercise ${exNum} NCERT Solutions | SJMaths`;
    } else if (relPath.includes('previous-years-questions')) {
      const topicName = filename.replace(/-/g, ' ').replace(/\b\w/g, c => c.toUpperCase());
      cleanTitle = `Class 12 ${chapName} - ${topicName} PYQs & Solutions | SJMaths`;
      ...
      cleanTitle = `Class 12 ${chapName} - ${testName} Practice Paper | SJMaths`;
    }
  ...
  if (!desc || desc.length < 50 || desc.length > 170) {
    ...
    desc = `Practice ${topicText} for Class 12 CBSE Board exam preparation. Access step-by-step solutions, key formulas, and expert revision notes on SJMaths.`;
    if (descM) {
  ...
  if (!html.includes('"BreadcrumbList"')) {
    const breadcrumbItems = [
      ...
      { "@type": "ListItem", "position": 2, "name": "Class 12 Maths", "item": "https://sjmaths.com/class-12-maths/" },
    ...
    const schemaGraph = [
      ...
      },
      {
        ...
        "educationalLevel": "Class 12 CBSE",
        "url": canonical
      }
    ];
...
console.log(`🎉 Unlocked and SEO-optimized ${count} Class 12 pages for search indexation.`);
