...
for (const filePath of htmlFiles) {
  ...
  if (bodyM) {
    ...
    if (wordCount < 100 && !html.includes('class="ai-summary"')) {
      const pageTitle = (titleM ? titleM[1] : 'Class 9 Maths').replace(/\s*\|\s*SJMaths/i, '');
      const summaryBlock = `
    <!-- AI Summary Block -->
    <section class="ai-summary" style="max-width: 800px; margin: 20px auto; padding: 20px; background: #f9fafb; border-radius: 12px; border: 1px solid #e5e7eb;">
        ...
        <p style="color: #4b5563; line-height: 1.6;">Access comprehensive <strong>${pageTitle}</strong> on SJMaths. Practice CBSE board exam previous year questions, NCERT solutions, exemplar problems, key formulas, and quick revision notes designed for Class 9 Mathematics preparation.</p>
    </section>\n`;
...
console.log(`🎉 Polished Class 9 titles, descriptions, and static summaries in ${count} files.`);
