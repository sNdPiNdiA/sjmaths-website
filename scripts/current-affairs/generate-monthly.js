...
const MONTHLY_DIR = path.join(__dirname, '..', '..', 'current-affairs', 'monthly');
...
const UI_TRANSLATIONS = {
  ...
  'monthly_magazine_title': { hi: 'मासिक समसामयिकी पत्रिका', en: 'Monthly Current Affairs Magazine' },
...
function main() {
  console.log('Generating Monthly Current Affairs Digests...');
  ...
  files.forEach(file => {
    ...
    monthGroups[monthId].push(dateStr);
  });

  ensureDir(MONTHLY_DIR);

  Object.keys(monthGroups).forEach(monthId => {
    const dateStrings = monthGroups[monthId];
    const newsItems = [];
    const mcqItems = [];

    // Collect all news and MCQs for this month
    ...
    // Group news articles topic-wise
    ...
    const mcqsHTML = mcqItems.slice(0, 25).map((q, idx) => `
    ...
    const contentHTML = `
      ${getBreadcrumbsAndToggle(`
        ...
        <a href="/current-affairs/monthly/">${t('monthly_summaries', 'Monthly Summaries')}</a> &gt; 
        <span>${monthId}</span>
      `)}
      ...
      <div style="background: rgba(142,68,173,0.02); border: 1px solid rgba(142,68,173,0.1); border-radius: 1.25rem; padding: 2rem; margin-bottom: 3rem;">
        ...
        ${categoryCompilationsHTML || `<p style="text-align: center; color: var(--text-light);">${t('no_highlights', 'No updates compiled for this month.')}</p>`}
      </div>

      <!-- MCQs -->
      <h2 style="font-family: 'Outfit', sans-serif; font-size: 1.6rem; color: var(--primary); margin: 3rem 0 1.5rem 0; border-bottom: 2px solid rgba(142,68,173,0.1); padding-bottom: 0.5rem;"><i class="fas fa-check-double"></i> ${t('monthly_mcqs', 'Monthly Revision Test')}</h2>
      <div class="ca-news-list">
        ${mcqsHTML || `<p style="text-align: center; color: var(--text-light);">${t('no_mcqs', 'No practice questions compiled for this month.')}</p>`}
      </div>
    `;

    const html = getHTMLTemplate(
      `Monthly Current Affairs Magazine — ${monthId}`,
      `Monthly current affairs magazine and compiled revision capsules for month ${monthId}. Free monthly general awareness updates and practice MCQs.`,
      `https://sjmaths.com/current-affairs/monthly/${monthId}/`,
      contentHTML
    );

    ensureDir(path.join(MONTHLY_DIR, monthId));
    fs.writeFileSync(path.join(MONTHLY_DIR, monthId, 'index.html'), html, 'utf8');
    console.log(`Generated monthly digest: ${monthId}`);
  });
}
