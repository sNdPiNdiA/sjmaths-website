...
const WEEKLY_DIR = path.join(__dirname, '..', '..', 'current-affairs', 'weekly');
...
function main() {
  console.log('Generating Weekly Current Affairs Digests...');
  ...
  // Load and group dates by week ID
  const weekGroups = {};
  files.forEach(file => {
  ...
  Object.keys(weekGroups).forEach(weekId => {
    const dateStrings = weekGroups[weekId];
    const newsItems = [];
    const mcqItems = [];

    // Collect all news and MCQs for this week
    ...
    const contentHTML = `
      ${getBreadcrumbsAndToggle(`
        ...
        <a href="/current-affairs/weekly/">${t('weekly_revision_notes', 'Weekly Revision Notes')}</a> &gt; 
        <span>${weekId}</span>
      `)}
      ...
      <div style="background: rgba(142,68,173,0.02); border: 1px solid rgba(142,68,173,0.1); border-radius: 1.25rem; padding: 2rem; margin-bottom: 3rem;">
        <h2 style="font-family: 'Outfit', sans-serif; font-size: 1.6rem; color: var(--text-dark); margin-bottom: 1.5rem;"><i class="far fa-list-alt"></i> ${t('weekly_capsules', 'Weekly Revision Capsules')}</h2>
        ${categoryCompilationsHTML || `<p style="text-align: center; color: var(--text-light);">${t('no_highlights', 'No updates compiled for this week.')}</p>`}
      </div>

      <!-- MCQs -->
      <h2 style="font-family: 'Outfit', sans-serif; font-size: 1.6rem; color: var(--primary); margin: 3rem 0 1.5rem 0; border-bottom: 2px solid rgba(142,68,173,0.1); padding-bottom: 0.5rem;"><i class="fas fa-check-double"></i> ${t('weekly_mcqs', 'Weekly Practice MCQs')}</h2>
      <div class="ca-news-list">
        ${mcqsHTML || `<p style="text-align: center; color: var(--text-light);">${t('no_mcqs', 'No practice questions compiled for this week.')}</p>`}
      </div>
    `;

    const html = getHTMLTemplate(
      `Weekly Current Affairs Digest — ${weekId}`,
      `Weekly current affairs digest and compiled revision capsules for week ${weekId}. Free weekly general awareness updates and practice MCQs.`,
      `https://sjmaths.com/current-affairs/weekly/${weekId}/`,
      contentHTML
    );

    ensureDir(path.join(WEEKLY_DIR, weekId));
    fs.writeFileSync(path.join(WEEKLY_DIR, weekId, 'index.html'), html, 'utf8');
    console.log(`Generated weekly digest: ${weekId}`);
  });
}
