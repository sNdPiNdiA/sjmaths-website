...
async function main() {
  ...
  const heads = await pool(publicUrls, async canonicalUrl => {
  ...
  const samples = chooseSamples(sitemapGroups);
  const sampledPages = await pool(samples, async canonicalUrl => {
    ...
    try {
      ...
      return { url, canonicalUrl, status: response.status, bytes: Buffer.byteLength(source), checks };
    } catch (error) {
  ...
  const result = {
    generatedAt: new Date().toISOString(), base: base.href, canonicalOrigin, previewNoindex,
