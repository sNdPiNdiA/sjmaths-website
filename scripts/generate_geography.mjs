...
/**
 * ============================================================================
 * SJ Maths — Geography Generator (Chemistry/Topic-Page Standard)
 * Styled with /assets/css/topic-page.min.css & Earth Emerald Palette
 * 5 Modular Tabs:
 *   1. 📖 Study Notes (Pillars, Physical/Human Mechanisms, Formulas/Scales, Mnemonics)
 *   2. ⚡ Revision Summary (Key Geographic Concepts, Terminology & Models)
 *   3. ❓ Practice Quiz (18-20 Interactive MCQs with Rationale)
 *   4. 🏛️ PYQs & Trends (UP TGT / UP PGT Geography Benchmarks)
 *   5. ⏱️ Timed Topic Test (10 Questions, 10-Minute Timer, Score Modal)
 * ============================================================================
 */
...
const STATUS_FILE = 'content-generation-status-geography.json';
...
function getUrlToTopicsMap() {
  const urlToTopics = {};
  if (fs.existsSync('up-pgt-geography/index.html')) {
    const pgtHtml = fs.readFileSync('up-pgt-geography/index.html', 'utf8');
  ...
  if (fs.existsSync('up-tgt-social-science/index.html')) {
    const tgtHtml = fs.readFileSync('up-tgt-social-science/index.html', 'utf8');
...
function getAllGeographyPages(dir = 'geography') {
  let pages = [];
  const list = fs.readdirSync(dir, { withFileTypes: true });
  for (const item of list) {
    const full = path.join(dir, item.name);
    if (item.isDirectory()) {
      pages = pages.concat(getAllGeographyPages(full));
...
function getTopicContext(topicUrl) {
  const parts = topicUrl.split('/').filter(Boolean); // ['geography', 'physical-geography', 'lithosphere', 'earthquakes']
...
function buildCall1Prompt(context) {
  ...
  return `You are a distinguished Professor of Geography and Chief Academic Examiner for UP TGT/PGT Geography and UGC-NET.
Produce deep, rigorous, exhaustive, academic-grade Study Notes and conceptual frameworks for the topic:
Topic: "${context.topicName}"
Section/Branch: "${context.sectionTitle}"${subtopicsText}
Exam Targets: UP PGT Geography (Subject Code 03), UP TGT Social Science, UGC-NET.

Return ONLY a valid, parseable JSON object matching this structure:
{
  "title": "${context.topicName}: Comprehensive Geographic Analysis, Physical & Human Mechanisms, and Regional Distribution",
  "short_intro": "High-yield conceptual breakdown and physical/human geographical foundations of ${context.topicName} tailored for competitive exam mastery.",
  "academic_synopsis": "Exhaustive 2-3 paragraph academic overview detailing the foundational definitions, geomorphic/meteorological/economic mechanisms, spatial variations, and exam relevance.",
  "conceptual_pillars": [
    {
      "pillar_title": "Title of Major Conceptual Pillar (e.g. Causes & Mechanism / Spatial Patterns / Theories)",
      "lead_concept": "Key theoretical law, geomorphic principle, or spatial model.",
      ...
      "comparative_insights": "Direct comparison with related phenomena or competing theories (e.g., Davis vs Penck, P-waves vs S-waves, Tropical vs Temperate cyclones).",
      "key_takeaways": [
  ...
  "spatial_and_regional_distribution": {
    "global_patterns": "Detailed global spatial distribution, major belts, tectonic plates, or climatic realms.",
    "indian_context": "Specific regional distribution in India, physiographic zones, states, river basins, or mineral corridors."
  },
  "formulas_and_scales": [
    {
      "name": "Scale or Mathematical Formulation Name (e.g., Richter Magnitude, Gradient Calculation, Demographic Indices)",
      "equation": "Mathematical formula or qualitative scale expression (e.g. M = log10(A) + C)",
      "parameters": "Explanation of each parameter and units of measurement",
      "exam_significance": "Why this formula/scale appears in competitive exams and typical calculation tricks"
    }
  ],
  "tricks_and_mnemonics": [
    {
      ...
      "type": "Mnemonic",
      "mnemonic": "Memorable acronym or phonetic phrase (English or Hindi)",
      "explanation": "Clear breakdown of what each letter/word stands for",
      "exam_pitfall_warning": "Common mistake or distractor trap set by examiners"
    }
  ]
}
...
4. Include at least 2 formulas, measurement scales, or index classifications.
...
function buildCall2Prompt(context, call1Data) {
  return `You are a Senior Question Setter for UP PGT Geography, UP TGT, and UGC-NET.
...
Return ONLY a valid, parseable JSON object matching this structure:
{
  "quick_revision": {
    ...
    "high_yield_laws_and_theories": [
      {
        "theorist_or_law": "Name of Scholar or Law (e.g. W.M. Davis, Wegener, Koppen, Burgess)",
        "year_or_period": "Year/Period (e.g. 1899, 1912, 1918)",
        "core_postulate": "Core postulate, hypothesis, or spatial diagram summary"
      }
    ]
  },
  "practice_quiz": [
    {
      "question": "Clear, rigorous MCQ testing conceptual understanding",
  ...
  "pyqs": [
    {
      "exam_year": "UP PGT Geography (Previous Year Pattern)",
  ...
  "topic_test": [
    {
      "question": "Rigorous timed test question testing analytical/map/factual mastery",
      "options": ["Option A", "Option B", "Option C", "Option D"],
      "correct_index": 2,
...
CRITICAL RULES:
1. Provide 12-15 glossary terms and 3-5 scholar/theory models.
...
function compileTopicHtml(call1, call2, context) {
  ...
  const metaDesc = `${call1.title}. Detailed study notes, geomorphic & meteorological mechanisms, formula sheets, mnemonics, PYQs, and topic test for UP TGT & PGT Geography.`;
  ...
  if (call1.formulas_and_scales && call1.formulas_and_scales.length > 0) {
    ...
    formulasHtml = `
      <div class="formula-sheet-box">
        <div class="formula-sheet-title">📐 Key Geographic Formulas, Scales & Classifications</div>
  ...
  if (call1.spatial_and_regional_distribution) {
    spatialHtml = `
      <section class="notes-section" id="spatial-distribution">
        <h2>Spatial Patterns & Regional Distribution</h2>
        <div class="prose-content">
          <div style="margin-bottom:1rem;">
            <h3 style="font-size:1.1rem;color:var(--brand);margin-bottom:0.4rem;">🌐 Global Geographic Belts & Spatial Variation</h3>
            <p>${call1.spatial_and_regional_distribution.global_patterns || ''}</p>
          </div>
          <div>
            <h3 style="font-size:1.1rem;color:var(--brand);margin-bottom:0.4rem;">🇮🇳 Indian Subcontinent Context & Regional Corridors</h3>
  ...
  if (glossary.length > 0) {
    ...
    glossaryHtml = `
      <div class="card" style="margin-bottom:1.5rem;">
        <h3 style="margin-bottom:1rem;color:var(--brand);font-size:1.2rem;">📚 High-Yield Geographic Terminology & Definitions</h3>
        <div>${items}</div>
      </div>
    `;
  }
  ...
  if (theories.length > 0) {
    ...
    theoriesHtml = `
      <div class="card">
        <h3 style="margin-bottom:1rem;color:var(--brand);font-size:1.2rem;">🧭 Prescribed Scholars, Laws & Spatial Models</h3>
        <div>${items}</div>
      </div>
    `;
  }
    ...
    pyqCardsHtml += `
      <div class="pyq-card" id="pyq-card-${idx}" data-correct="${q.correct_index ?? 0}">
        <div class="pyq-header">
          <span class="exam-tag">${q.exam_year || 'UP PGT/TGT Geography'}</span>
        </div>
        <div class="question-text">${q.question}</div>
        <div class="quiz-options-grid">${opts}</div>
...
<title>${call1.title} | UP TGT &amp; PGT Geography</title>
...
<!-- Geography Emerald & Earth Theme Override -->
<style>
:root {
  --brand: #064e3b;
  --brand-dark: #022c22;
  ...
  <div class="wrap header-inner">
    <a class="brand" href="https://sjmaths.com/">
      ...
      <span>
        ...
        <span class="brand-sub">Geography</span>
      </span>
    </a>
    <div class="header-actions">
      <button type="button" class="theme-toggle-btn" id="btn-theme-toggle" aria-label="Toggle Dark Mode">🌙 Dark Mode</button>
      <a class="back-btn" href="/up-pgt-geography/" title="UP PGT Geography Tracker">← UP PGT<span class="desk-only"> Geography</span></a>
    </div>
  </div>
</header>
...
<main class="wrap">
  ...
  <section class="hero">
    <nav class="breadcrumb" aria-label="Breadcrumb">
      ...
      <a href="/up-pgt-geography/">Geography</a>
      <span class="breadcrumb-sep">›</span>
    ...
    <div class="exam-badges">
      <a class="exam-chip pgt" href="/up-pgt-geography/">UP PGT Geography</a>
      <a class="exam-chip both" href="/up-tgt-social-science/">UP TGT Social Science</a>
  ...
  <div class="main-grid">
    <div class="content-col">
      ...
      <article class="tab-panel hidden" id="tab-quiz" role="tabpanel" aria-labelledby="tab-btn-quiz">
        <div class="quiz-summary-card">
          ...
          <p>Instant evaluation with detailed spatial &amp; physical explanations for each question.</p>
        </div>
      ...
      <article class="tab-panel hidden" id="tab-test" role="tabpanel" aria-labelledby="tab-btn-test">
        ...
        <div class="test-result-modal hidden" id="testResultModal">
          <div class="result-card">
            ...
            <p id="resFeedbackText">Review detailed geographic solutions above for all questions.</p>
      ...
      <nav class="topic-pagination" aria-label="Topic Navigation" style="margin-top:2rem;">
        <a class="topic-nav-btn next" href="/up-pgt-geography/"><span>Exam Tracker →</span> <strong>UP PGT Geography</strong></a>
      </nav>
    ...
    <aside class="sidebar-col">
      <div class="sidebar-card side-card">
        ...
        <p class="side-desc">Official UP PGT Geography curriculum module.</p>
        <div class="side-action-box" style="margin-top:1.5rem;">
          <a class="side-action-btn" href="/up-pgt-geography/" style="background: #064e3b; display:block; text-align:center; padding:10px; color:#fff; border-radius:8px; font-weight:600;">Full Geography Tracker →</a>
        </div>
      </div>
    </aside>
  </div>
...
<footer class="site-footer">
  <div class="wrap footer-inner">
    <div>
      <p><strong>SJ Maths — Geography</strong></p>
      <p>Master curriculum resources for UP PGT Geography &amp; UP TGT Social Science.</p>
    </div>
    <div class="footer-links">
      <a href="https://sjmaths.com/">Home</a>
      <a href="/up-pgt-geography/">UP PGT Geography</a>
...
async function processTopic(topicUrl) {
  ...
  console.log(`\n========================================`);
  console.log(`Processing Geography Topic: ${topicUrl}`);
...
async function main() {
  const allPages = getAllGeographyPages();
  console.log(`Discovered ${allPages.length} active Geography chapter pages on disk.`);
