...
const targetSubdirs = [
  path.join(CLASS9_DIR, 'ncert-exercise-practice'),
  path.join(CLASS9_DIR, 'ncert-examplar-practice'),
  path.join(CLASS9_DIR, 'worksheets'),
  path.join(CLASS9_DIR, 'tests')
];
...
console.log(`Found ${unlockedFiles.length} Class 9 target pages to unlock for indexing.`);
...
for (const filePath of unlockedFiles) {
  ...
  if (!title || title.length < 25 || title.includes('Updating')) {
    ...
    if (relPath.includes('ncert-exercise-practice')) {
      ...
      cleanTitle = `Class 9 Maths Chapter ${chapName} Exercise ${exNum} NCERT Solutions | SJMaths`;
    } else if (relPath.includes('ncert-examplar-practice')) {
      const topicName = filename.replace(/-/g, ' ').replace(/\b\w/g, c => c.toUpperCase());
      cleanTitle = `Class 9 ${chapName} ${topicName} NCERT Exemplar Solutions | SJMaths`;
    } else if (relPath.includes('worksheets')) {
      const type = filename.replace(/-/g, ' ').replace(/\b\w/g, c => c.toUpperCase());
      cleanTitle = `Class 9 ${chapName} ${type} Worksheet Solutions | SJMaths`;
    } else if (relPath.includes('tests')) {
      const testName = filename.replace(/-/g, ' ').replace(/\b\w/g, c => c.toUpperCase());
      cleanTitle = `Class 9 ${chapName} ${testName} Online Test | SJMaths`;
    }
  ...
  if (!desc || desc.length < 50 || desc.length > 170) {
    ...
    desc = `Practice ${topicText} for Class 9 CBSE Board exam preparation. Access step-by-step solutions, key formulas, and expert revision notes on SJMaths.`;
    if (descM) {
  ...
  if (!html.includes('"BreadcrumbList"')) {
    const breadcrumbItems = [
      ...
      { "@type": "ListItem", "position": 2, "name": "Class 9 Maths", "item": "https://sjmaths.com/class-9-maths/" },
...
console.log(`🎉 Unlocked and SEO-optimized ${count} Class 9 pages for search indexation.`);
