...
console.log(`Starting consolidation & de-nesting for ${config.length} groups...`);
...
for (const group of config) {
  ...
  // Remove superseded source folders
  for (const src of group.sources) {
...
// Clean up empty directories under cartography/remote-sensing-gis if empty
const rsgisDir = path.join(process.cwd(), 'geography/cartography/remote-sensing-gis');
if (fs.existsSync(rsgisDir)) {
  const contents = fs.readdirSync(rsgisDir);
  if (contents.length === 0) {
    fs.rmdirSync(rsgisDir);
    console.log('Removed empty directory: geography/cartography/remote-sensing-gis');
  }
}

console.log(`\nDe-nesting complete! Targets created/updated: ${totalTargetsCreated}, Source folders cleaned up: ${totalSourcesRemoved}`);
