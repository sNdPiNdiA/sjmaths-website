/**
 * generate-ssc-cgl-physics-html.js
 * Generates 6 bilingual Physics topic pages for SSC CGL General Awareness.
 * Output: ssc-cgl/general-awareness/physics/<slug>/index.html
 *
 * Improvements over economy generator:
...
// ── Topics ───────────────────────────────────────────────────────────────────
const TOPICS = [
    { name: 'Units, Dimensions & Errors',                       slug: 'units-dimensions-and-errors' },
    { name: "Newton's Laws of Motion & Gravitation",            slug: 'newtons-laws-of-motion-and-gravitation' },
    { name: 'Work, Energy & Power',                             slug: 'work-energy-and-power' },
    { name: 'Thermodynamics, Heat Transmission & Sound Waves',  slug: 'thermodynamics-heat-transmission-and-sound-waves' },
    { name: 'Light, Optics & Lens Devices',                     slug: 'light-optics-and-lens-devices' },
    { name: 'Electricity, Magnetism & Radioactivity',           slug: 'electricity-magnetism-and-radioactivity' },
];

const OUTPUT_DIR = path.join(
    __dirname,
    'ssc-cgl', 'general-awareness', 'physics'
);
...
function buildPrompt(topicName) {
    return `You are the Senior Physics Faculty and Content Team of SJMaths.
...
3. Start from absolute beginner level and progress to SSC CGL exam level.
   ...
   Highlight important physical constants, formulae, SI units, and landmark discoveries.
...
4. CONTENT STRUCTURE for Tab 1 (Concept & Theory) — include ALL applicable sections:
   ...
   - Key Formulas / Physical Constants / SI Units (premium-table)
   - Examples & Numerical Illustrations (exam-style)
...
6. CRITICAL MATH RULE — lang-en sections:
   ...
   - CORRECT: $$F = ma$$, $$E = mc^2$$, $$v = u + at$$
   - NEVER: $$\\text{बल} = \\text{द्रव्यमान} \\times \\text{त्वरण}$$
7. CRITICAL MATH RULE — lang-hi sections:
   ...
   - CORRECT (Hindi): <p class="lang-hi">बल (F) = द्रव्यमान (m) × त्वरण (a)</p>`;
}
...
<head>
    ...
    <title>${safeTitle} - SSC CGL Physics Notes | SJMaths</title>
    <meta name="description" content="Master ${safeTitle} for SSC CGL General Awareness. Bilingual concept notes, formulas, SI units, practice questions, and exam tips.">
    <meta name="robots" content="index, follow, max-image-preview:large">
    <link rel="canonical" href="https://sjmaths.com/ssc-cgl/general-awareness/physics/${slug}/">
    <meta name="keywords" content="${safeTitle}, SSC CGL Physics, General Awareness, SSC CGL Tier 1, SJMaths">
    ...
    <style>
        :root {
            --phy-grad: linear-gradient(135deg, #2980b9, #16a085);
            --glass-bg: rgba(255,255,255,0.96);
            --glass-border: rgba(255,255,255,0.25);
            --shadow-lg: 0 10px 30px -5px rgba(41,128,185,0.12);
        }
        ...
        .topic-header h1 { font-family: 'Outfit', sans-serif; font-size: clamp(1.5rem,5vw,2.2rem); font-weight: 800; background: var(--phy-grad); -webkit-background-clip: text; -webkit-text-fill-color: transparent; margin-bottom: .4rem; line-height: 1.2; }
        ...
        .tab-btn:hover { color: #2980b9; background: rgba(41,128,185,.05); }
        .tab-btn.active { color: #fff; background: var(--phy-grad); box-shadow: 0 4px 10px rgba(41,128,185,.25); }
        ...
        .coming-soon-box { text-align: center; padding: 3rem 1.5rem; background: linear-gradient(135deg, rgba(41,128,185,.03), rgba(22,160,133,.03)); border-radius: 1rem; border: 2px dashed rgba(41,128,185,.2); }
        .coming-soon-box i { font-size: 3rem; color: #2980b9; margin-bottom: 1rem; opacity: .6; }
        ...
        .tab-panel h2 { font-family: 'Outfit', sans-serif; font-size: 1.35rem; font-weight: 700; color: #2980b9; margin-top: 1.25rem; margin-bottom: .85rem; border-bottom: 2px solid rgba(41,128,185,.15); padding-bottom: .35rem; }
        ...
        .premium-table th { background: rgba(41,128,185,.08); font-weight: 700; color: #2980b9; padding: 12px 14px; border-bottom: 2px solid rgba(41,128,185,.15); white-space: nowrap; }
        ...
        .premium-table tr:hover td { background: rgba(41,128,185,.03); }
...
<body>
    ...
    <main class="topic-container" id="main-content">
        <div class="breadcrumbs">
            ...
            <a href="/ssc-cgl/general-awareness/physics/">Physics</a> <i class="fas fa-chevron-right" style="font-size:.65rem;margin:0 .2rem;"></i>
            <span>${safeTitle}</span>
        </div>
        <div class="topic-header">
            <h1>${safeTitle}</h1>
            <p><span class="lang-en">Complete SSC CGL Physics Guide: Concepts, Formulas, PYQs &amp; Mock Test</span><span class="lang-hi">संपूर्ण SSC CGL भौतिकी गाइड: अवधारणाएं, सूत्र, PYQ और मॉक टेस्ट</span></p>
        </div>
...
function latexToPlainHtml(latex, isDisplay) {
    let r = latex
        ...
        .replace(/\\infty/g, '∞').replace(/\\sqrt\{([^}]*)\}/g, '√($1)')
...
// ── Main ─────────────────────────────────────────────────────────────────────
async function main() {
    console.log('🔬 SSC CGL Physics Page Generator\n');
    ...
    for (let i = 0; i < TOPICS.length; i++) {
        ...
        try {
            ...
            let html = buildHTML(name, slug, tab1);
