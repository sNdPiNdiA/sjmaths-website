...
async function translateMetadata(client, name, description) {
    const prompt = `You are a translator. Translate the following UPSC Geography microtopic name and description into Hindi.
Name: "${name}"
...
function buildMeta(parentDir, parentName, microtopicDir, name, description, hindiName = '', hindiDescription = '', previousTopic = null, nextTopic = null) {
    return {
        ...
        confusedDirs: [],
        confusedTopicsHi: [],
        canonicalUrl: `https://sjmaths.com/upsc/geography/${parentDir}/${microtopicDir}/`,
        description,
        hindiDescription: hindiDescription || (name + ' पर UPSC GS-3 की विस्तृत गाइड।'),
        category: 'GS-3',
        supportsMains: true,
        topicId: `geography.${parentDir.toLowerCase()}.${microtopicDir.toLowerCase()}`,
...
async function main() {
    console.log('╔══════════════════════════════════════════════════════════════╗');
    console.log('║ UPSC Geography Microtopic Batch Generator                  ║');
    console.log('╚══════════════════════════════════════════════════════════════╝\n');
    ...
    const subjectDir = path.join(process.cwd(), 'upsc', 'geography');
    ...
    console.log('\nAll Geography microtopics processed successfully!');
}
