...
function kebabToTitle(kebab) {
    return kebab.split('-').map(word => {
        ...
        // Common environment acronyms
        const acronyms = [
            'EIA', 'UNFCCC', 'IPCC', 'IUCN', 'CBD', 'COP', 'GEF', 'UNEP', 'WMO', 'WWF', 'CITES', 
            'TRAFFIC', 'CAWT', 'CMS', 'CZA', 'NTCA', 'WCCB', 'WTI', 'EPA', 'NDMA', 'GIS', 'GPS', 
            'NGO', 'ESG', 'NPP', 'GPP', 'BOD', 'COD', 'SDG', 'PM', 'SPM', 'AQI', 'GHG', 'ODS', 
            'CFC', 'HCFC', 'HFC', 'PFC', 'SF6', 'NF3', 'CO2', 'CH4', 'N2O', 'O3', 'VOC', 'POP', 
            'PCB', 'DDT', 'PAH', 'PAN', 'REDD', 'CDM', 'JI', 'INDC', 'NDC', 'NAPCC', 'SAPCC', 
            'ISA', 'CDRI', 'OSOWOG', 'LULUCF'
        ];
...
async function translateMetadata(client, name, description) {
    const prompt = `You are a translator. Translate the following UPSC Environment microtopic name and description into Hindi.
Name: "${name}"
...
function buildMeta(parentDir, parentName, microtopicDir, name, description, hindiName = '', hindiDescription = '', previousTopic = null, nextTopic = null) {
    return {
        ...
        confusedDirs: [],
        confusedTopicsHi: [],
        canonicalUrl: `https://sjmaths.com/upsc/environment/${parentDir}/${microtopicDir}/`,
        description,
        hindiDescription: hindiDescription || (name + ' पर UPSC GS-3 की विस्तृत गाइड।'),
        category: 'GS-3',
        supportsMains: true,
        topicId: `environment.${parentDir.toLowerCase()}.${microtopicDir.toLowerCase()}`,
        practiceTypes: ['basic', 'conceptual'],
        difficulty: 'medium',
        studyTime: { concepts: 20, practice: 15 },
        learningObjectives: [
            `Understand key UPSC concepts for ${name}`,
            `Memorize key facts and ecological significance for ${name}`,
            `Apply to UPSC Prelims and Mains (GS-3)`
        ],
        scope: {
            mustExplain: [name, 'Environment', 'UPSC GS-3'],
            mayMention: ['Current Affairs', 'Government Initiatives', 'Conservation Efforts'],
            neverExplain: ['Ancient History', 'Polity', 'Economy'],
            relatedTopics: [name],
            keywords: ['UPSC', 'Environment', 'Ecology', name]
        },
        related: {
...
async function main() {
    console.log('╔══════════════════════════════════════════════════════════════╗');
    console.log('║ UPSC Environment Microtopic Batch Generator                ║');
    console.log('╚══════════════════════════════════════════════════════════════╝\n');
    ...
    const subjectDir = path.join(process.cwd(), 'upsc', 'environment');
    ...
    for (const parentDir of parentDirs) {
        ...
        for (const childDir of childDirs) {
            ...
            let description = `Study guide for ${name} under ${parentName} for UPSC Civil Services Prep (GS-3 Environment & Ecology).`;
    ...
    console.log('\nAll Environment microtopics processed successfully!');
}
