...
function kebabToTitle(kebab) {
    return kebab.split('-').map(word => {
        ...
        // Common art-and-culture acronyms
        const acronyms = ['AI', 'ICT', 'IT', 'IoT', 'DNA', 'RNA', 'IPR', 'BT', 'LASER', 'WIFI', 'USB', 'PCR',
            'ISRO', 'NASA', 'GPS', 'IRNSS', 'GSAT', 'INSAT', 'PSLV', 'GSLV', 'LVM', 'DRDO', 'BARC', 'DAE',
            'DST', 'DBT', 'ASI', 'INTACH', 'UNESCO', 'ICC', 'NSD', 'SPIC-MACAY'];
...
async function translateMetadata(client, name, description) {
    const prompt = `You are a translator. Translate the following UPSC Art & Culture microtopic name and description into Hindi.
Name: "${name}"
...
JSON:`;
    try {
        ...
        return {
            ...
            hindiDescription: parsed.hindiDescription || (parsed.hindiName + ' पर UPSC GS-1 की विस्तृत गाइड।')
        };
    } catch (err) {
        return {
            hindiName: name,
            hindiDescription: name + ' par UPSC GS-1 ki vistrit guide.'
        };
    }
}
...
function buildMeta(parentDir, parentName, microtopicDir, name, description, hindiName = '', hindiDescription = '', previousTopic = null, nextTopic = null) {
    return {
        ...
        confusedDirs: [],
        confusedTopicsHi: [],
        canonicalUrl: `https://sjmaths.com/upsc/art-and-culture/${parentDir}/${microtopicDir}/`,
        description,
        hindiDescription: hindiDescription || (name + ' पर UPSC GS-1 की विस्तृत गाइड।'),
        category: 'GS-1',
        supportsMains: true,
        topicId: `art-and-culture.${parentDir.toLowerCase()}.${microtopicDir.toLowerCase()}`,
        practiceTypes: ['basic', 'conceptual'],
        difficulty: 'medium',
        studyTime: { concepts: 20, practice: 15 },
        learningObjectives: [
            `Understand key UPSC concepts for ${name}`,
            `Memorize key facts and cultural significance for ${name}`,
            `Apply to UPSC Prelims and Mains (GS-1)`
        ],
        scope: {
            mustExplain: [name, 'Art & Culture', 'UPSC GS-1'],
            mayMention: ['Current Affairs', 'Government Initiatives', 'Cultural Heritage'],
            neverExplain: ['Ancient History', 'Polity', 'Economy'],
            relatedTopics: [name],
            keywords: ['UPSC', 'Art', 'Culture', name]
        },
        related: {
...
async function main() {
    console.log('╔══════════════════════════════════════════════════════════════╗');
    console.log('║ UPSC Art & Culture Microtopic Batch Generator              ║');
    console.log('╚══════════════════════════════════════════════════════════════╝\n');
    ...
    const subjectDir = path.join(process.cwd(), 'upsc', 'art-and-culture');
    ...
    for (const parentDir of parentDirs) {
        ...
        for (const childDir of childDirs) {
            ...
            let description = `Study guide for ${name} under ${parentName} for UPSC Civil Services Prep (GS-1 Art & Culture).`;
    ...
    console.log('\nAll Art & Culture microtopics processed successfully!');
}
